<template>
    <div class="permission-wrap">
        ...
        <Row>
            <Col span="24">
            <Table border ref="selection" :columns="roleColumns" :data="rolesData" style="margin:20px;"></Table>
            <Modal v-model="visableModal" :title="this.ModalType === 'new' ? '添加角色' : '修改角色'" @on-ok="addOrEditRole">
                <Form :model="role" :label-width="80">
                    ...
                    <FormItem label="备注">
                        <Input v-model="role.desc" type="textarea" :autosize="{minRows: 2,maxRows: 5}"
                            placeholder="请输入角色备注"></Input>
                    </FormItem>
                    <FormItem label="权限">
